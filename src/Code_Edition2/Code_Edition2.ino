...
void setup(void) {
  ...
  // Create tasks
  xTaskCreate(taskDigitalSignalOutput, "DigitalOutput", 4096, NULL, 1, NULL); // Task1 the priority of Task1 can't higner than other
  ...
  xTaskCreate(taskCPULoad, "CPULoad", 1024, NULL, 1, NULL);                   // Task8
...
void taskMeasureFrequency2(void *pvParameters) {
    ...
    while(1) {
        // stack test
        uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        Serial.print("Task 2 Stack High Water Mark: ");
        Serial.println(uxHighWaterMark);
...
void taskMeasureFrequency3(void *pvParameters) {
    ...
    while(1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 3 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
        vTaskDelayUntil(&xLastWakeTime, xFrequencyMeasurePeriod);
        hasCalculated2 = 0;
...
void taskSampleAnalogInput(void *pvParameters) {
    ...
    while(1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 4 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
        vTaskDelayUntil(&xLastWakeTime, xFrequencyMeasurePeriod);
...
void taskLogInformation(void *pvParameters) {
    ...
    while (1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 5 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
...
void taskMonitorPushButton(void *pvParameters) {
    ...
    while (1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 7-1 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
...
void taskControlLED(void *pvParameters) {
    ...
    while (1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 7-2 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
        if (xQueueReceive(buttonPressEventQueue, &receivedSignal, portMAX_DELAY) == pdPASS) {
            ...
            ledToggleTime = micros(); // Record the timestamp of the LED switch
            responseTime = ledToggleTime - buttonPressTime; // Calculated response time
...
void taskCPULoad(void *pvParameters) {
    ...
    while (1) {
        // uxHighWaterMark = uxTaskGetStackHighWaterMark(NULL);
        // Serial.print("Task 8 Stack High Water Mark: ");
        // Serial.println(uxHighWaterMark);
        CPU_work(2); 
